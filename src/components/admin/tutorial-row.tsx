...
export function AdminTutorialRow({ tutorial }: AdminTutorialRowProps) {
  ...
  const [featured, setFeatured] = useState(tutorial.featured);

  const handlePublish = () => {
    const next = !published;
    setPublished(next);
    startTransition(() => toggleTutorialPublishedAction(tutorial.slug, next));
  };

  const handleFeatured = () => {
    const next = !featured;
    setFeatured(next);
    startTransition(() => toggleTutorialFeaturedAction(tutorial.slug, next));
  };
  ...
  const formattedDate = new Date(tutorial.date).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  });
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-start flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="font-medium text-foreground line-clamp-1">{tutorial.title}</span>
          </div>
          <span className="text-xs text-muted-foreground font-mono pl-3.5">/tutorials/{tutorial.slug}</span>
        </div>
      </td>
